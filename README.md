...
Each flow name is optional. A missing or empty value uses the original
default from the `3.2.0` branch:
...
only, matching the behavior of the `3.2.0` branch. When it is `true`, GitHub
tags are added to the available refs for the model, backend, and frontend
repositories. Duplicate branch and tag names are shown only once.

The flow modules look for these options in `config/prefect-ins.yaml`. If the
file or the `prefect_options` section is absent, all original `3.2.0`
